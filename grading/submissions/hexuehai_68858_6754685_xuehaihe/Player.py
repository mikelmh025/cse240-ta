...
class AIPlayer:
    def __init__(self, player_number):
        ...
        self.type = type
        self.player_string = f"Player {player_number}:{type}"
        self.ctoScore = {
            1: 1,
            2: 11,
            3: 111,
    ...
    @staticmethod
    def connect_num(board, direction=Direction.HORIZON):
            ...
            for i in range(board.shape[0]):
                ...
                for j in range(1, board.shape[1]):
                    if s == board[i][j]:
                        ...
                        if s != 0:
                            player_to_cnt_list.append((s, cnt, actions))
                        s = board[i][j]
                        cnt = 1
                        actions=[(i, j)]
    ...
    def _max_value(self, player, board, depth, alpha, beta):
        ...
        cc = [(self._connectNum(board, direction), direction) for direction in [Direction.HORIZON, Direction.VERTICAL, Direction.DIALOG, Direction.RDIALOG]]
        if depth == 0 or len(next_move) == 0 or self._isOver(cc):
            if depth == 0:
                self.tree_visited += 1
                logger.info(f"Finished one tree with depth = 5, tree_visited = {self.tree_visited}")
